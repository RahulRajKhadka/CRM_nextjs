...
export default function OurServicesPage() {
  ...
  const handleAddService = () => {
    if (newService.name && newService.price) {
      ...
      setNewService({ name: '', description: '', category: 'Internet', price: '', status: 'active' });
      setShowModal(false);
    }
  };
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      <div className="flex-1 flex flex-col">
        <Header title="Our Services" userName="Dolma Gurung" userLocation="Dolma" />
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200">
                    <th className="text-left py-4 px-4 text-sm font-medium text-gray-600">Service Name</th>
                ...
                <tbody>
                  {filteredServices.map((service) => (
                    <tr key={service.id} className="border-b border-gray-100 hover:bg-gray-50">
                      <td className="py-4 px-4 text-sm font-medium text-gray-900">{service.name}</td>
                      <td className="py-4 px-4 text-sm text-gray-600">{service.description}</td>
                      <td className="py-4 px-4 text-sm text-gray-600">
                        <span className="px-2 py-1 bg-blue-100 text-blue-700 rounded-full text-xs">
                          {service.category}
                        </span>
                      </td>
                      <td className="py-4 px-4 text-sm font-semibold text-gray-900">{service.price}</td>
